...
* **wwSQL - Configurable Server= when not provided**  
The default server string has changed from `server=.` to `server=127.0.0.1` due to some changes how ODBC now detects local machines. The default value is now also configurable via a new `wwSQL_DEFAULT_SERVERNAME` compiler variable in `wconnect.h`. This allows you to set specific protocols like `lpc:.` or `np:.` or `tcp:127.0.0.1,1434` for your application via `wconnect_override.h` as a safe fallback when no explicit connection string is passed.
...
Returns the currently active SFTP directory name as a string.

* **Documentation Updates: Dark Mode added, Typography and Navigation Improvements**  
There are additional documentation improvements - courtesy of [Documentation Monster](https://documentationmonster.com) - including newly added support for Dark Mode in the documentation. Also many template improvements for cleaner topic typography and topic tree navigation improvements.
...
[Release Post](https://west-wind.com/wconnect/weblog/ShowEntry.blog?id=57037) &bull; 
